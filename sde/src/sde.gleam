...
const sde_types_filename = "types.yaml"
...
pub fn main() {
  io.println("Loading types...")
  let assert Ok([doc]) = yay.parse_file(sde_types_filename)
    as "Could not find 'types.yaml'. Download it from the Eve Online SDE archive at https://developers.eveonline.com/static-data"
...
fn make_ttype(node: #(yay.Node, yay.Node)) -> Result(Ttype, yay.ExtractionError) {
  io.println("Parsing item " <> string.inspect(node.0))
